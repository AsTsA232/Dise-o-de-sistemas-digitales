module regtst(
    ...
    logic [7:0]      two_digit_number;  // Registro para almacenar el número de dos dígitos
    ...
    always_ff @(posedge clk_fpga or posedge reset) begin
        if (reset) begin
            ...
            baudrate_counter<= 0;
            data_index      <= 0;
            two_digit_number <= 0;  // Reset del número
        end else begin
                ...
                if (sample_counter == div_sample - 1) begin
                    if (bit_counter == div_bit - 1) begin
                        ...
                        if (data_index == 2) begin
                            // Convertir los caracteres ASCII a números y combinarlos
                            two_digit_number <= (data_buffer[0] - "0") * 10 + (data_buffer[1] - "0");
                            // Limpiar el índice para recibir el siguiente número
                            data_index <= 0;
                        end
